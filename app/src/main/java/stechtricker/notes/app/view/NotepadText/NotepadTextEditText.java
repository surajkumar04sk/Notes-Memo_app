...
public class NotepadTextEditText extends EditText implements TextWatcher {
    ...
    public NotepadTextEditText(Context context) {
        super(context);
        init(null);
    }

    public NotepadTextEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public NotepadTextEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @SuppressWarnings("NewApi")
    public NotepadTextEditText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.NotepadTextView);
        bulletColor = array.getColor(R.styleable.NotepadTextView_bulletColor, 0);
        bulletRadius = array.getDimensionPixelSize(R.styleable.NotepadTextView_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.NotepadTextView_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.NotepadTextView_historyEnable, true);
        historySize = array.getInt(R.styleable.NotepadTextView_historySize, 100);
        linkColor = array.getColor(R.styleable.NotepadTextView_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.NotepadTextView_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.NotepadTextView_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.NotepadTextView_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.NotepadTextView_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<NotepadTextPart> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new NotepadTextPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (NotepadTextPart part : list) {
            if (part.isValid()) {
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new NotepadTextBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new NotepadTextQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new NotepadTextURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        builder.append(NotepadTextParser.fromHtml(source));
        switchToKnifeStyle(builder, 0, builder.length());
        setText(builder);
    }

    public String toHtml() {
        return NotepadTextParser.toHtml(getEditableText());
    }
    ...
    protected void switchToKnifeStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new NotepadTextBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new NotepadTextQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new NotepadTextURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
}
